...
def get_display_backend_flags(browser_key: str = "") -> str:
    ...
    Rules:
      • X11  → no extra flags needed; the browser's native X11 path is used.
      • Wayland + Chromium-based:
          ...
          - wlroots-based (sway, hyprland, river, labwc, wayfire):
            add --disable-gpu-sandbox workaround that helps on some wlroots setups
    ...
    if session == "x11":
        # Pure X11: Chromium uses XCB natively, no ozone flags needed.
        return ""
...
def make_launcher_wrapper(app: dict, browser_key: str, nice: int, ionice: int, gpu: bool) -> Path:
    ...
    # ── Snap name mapping (our keys differ from snap package names) ─────────
    _SNAP_NAMES = {
        "firefox":      "firefox",
        "edge":         "microsoft-edge",
        "brave":        "brave",
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with tarfile.open(backup_path, "r:gz") as tar:
                ...
                unsafe = [m.name for m in tar.getmembers() if not _safe_member(m, tmp)]
                if unsafe:
